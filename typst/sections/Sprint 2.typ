...
- Potentiometers via multiplexer
  - As mentioned in @sec:sprint1MusicalInteraction the four potentiometers would require more analog inputs than what the Pico 1's offered. A multiplexer was therefore added, allowing the Pico 1's to receive analog inputs from all four potentiometers using only one analog GPIO pin. This solution did, however, also require three digital GPIO pins.
- Power system
...
Four tactile switch buttons @adafruit_tactile_nodate were added to the breadboard, according to the schematic. Each button was connected to the Pico using an internal pull-up configuration as mentioned in @sec:sprint1MusicalInteraction. The buttons were implemented so that, when a button was pressed, the controller would send a single message to the host indicating which button had been activated (e.g., “Button 3 pressed”) (@listing:sprint2Buttons).
