...
- Display (Pico Display 2.0)
  - The display (@sprint1display) was also connected to the Pico 1's SPI-0 bus, but used a different _CSn_ pin. This allowed saving GPIO pins compared to using the SPI-1 bus.
- Buttons
...
- Potentiometers via multiplexer
  - The four potentiometers would require more analog inputs than what the Pico 1s offered (@sec:sprint1MusicalInteraction). A multiplexer @arduinotechdk_74hc4051_2025 was therefore added, allowing the Pico 1s to receive analog inputs from all potentiometers using only one analog GPIO pin. This solution did, however, also require three digital GPIO pins.
- Power system
  - Four AA batteries were added as the primary power source. This would result in a power supply of $4 dot 1.2V = 4.8V$, closely resembling the 5V the Pico 1s would receive when connected to a computer via USB.
  ...
  - A MOSFET transistor and a resistor was added in accordance with the recommendations of #cite(<raspberry_pi_raspberry_2024-1>, form: "prose") This was done so that the batteries would be automatically disconnected when the Pico 1s were connected to a computer for debugging, ensuring neither could be damaged.
- LEDs
  - Based on testing (@sec:sprint1test), ten _Breadboard-friendly RGB Smart NeoPixel_ @adafruit_breadboard-friendly_2025 LEDs were added to the schematic. These were chosen for their availability, pricing, and flexibility allowing the control of multiple RGB LEDs using only one GPIO pin and a first-party library made by Adafruit @george_adafruit_2025.
...
By following the schematic (@app:schematicSprint2), a Controller was successfully assembled on a breadboard to test, how the individual parts interacted. The LEDs were deemed not strictly important for the functionality of the Controller and was therefore not implemented on the breadboard.
...
if card_str in self.card_labels:
  ...
  ```,
  caption: [Function to load images integrated with NFC functionality.],
  placement: top
) <listing:NFCDisplay>
...
#figure(
  image("../images/sprint 2/HostBreadboard.jpg", height: 25%),
  caption: [Host breadboard prototype with two NeoPixels.],
) <fig:hostBreadboard>
...
#figure(
  ...
  ```,
  caption: [Function for NeoPixel alerting if a client connects or disconnects.],
...
#figure(
  image("../images/sprint 2/Ableton-2Tracks.png", height: 28%),
  caption: [Live setup for playing different instruments on different MIDI channels.]
...
#subpar.grid(
  ...
  align: top,
  figure(image("../images/sprint 2/DevicePlayingDiagram.png", height: 35%),
    caption: [Message flow when using Controller.]), <fig:DevicePlayingDiagram>,
  figure(image("../images/sprint 2/ChangeSoundDiagram.png", height: 35%),
...
#subpar.grid(
  ...
  label: <fig:fus2>,
  align: top,
  figure(image("../images/Fusion/fus2FirstAttemptBox.png", height: 18%),
    caption: [Attempt one at designing chassis.]), <fig:fus2FirstAttempt>,
  figure(image("../images/Fusion/fus2Assembled.png", height: 18%),
    caption: [Chassis after changes.]), <fig:fus2Assembled>,
  figure(image("../images/Fusion/3DFirstBox.jpg", height: 18%),
    caption: [3D printed chassis.]), <fig:3DFirstPrintedBox>,
    placement: top
)
