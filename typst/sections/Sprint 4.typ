...
Feedback from testing (@sec:sprint3test) informed the choice of redesigning the NFC card slot to prevent the NFC cards from sliding out. Additionally. Additionally, custom potentiometer knobs weredesigned. Furthermore,, automatic card detection was developed to alleviate the pain point testers had remembering to press the buttonactivatinging card detection. A switch debouncing algorithm was created for improved interaction with the system, and professionally manufactured PCBs were ordered to replace thethe earlier hand-made version, to resolve several issues.
#colbreak()
#colbreak()
...
#figure(
  ...
  ```,
  caption: [FuncinFunctions on Controller for handling of potentiometers.],
  placement: top,
  placement: top
) <listing:sprint4Pot>
...
#subpar.grid(
  ...
  align: top,
  figure(image("../images/sprint 4/Oscilloscope1.jpg", width: 990%),
    caption: [Testing and analyzing button's voltage waveform.]), <fig:oscilloscope1>,
  figure(image("../images/sprint 4/Oscilloscope2.jpg", width: 990%),
    caption: [Close-up of oscilloscope during debounce-analysis.]), <fig:oscilloscope2>,,
)
#colbreak()#colbreak()
== Automatic CCard DDetection
To enable automatic NFC card detection, instead of having to push a button, it was decided to add a sensor-based trigger to the NFC reading mechanism. This was due to testing revealing that the prior implementation caused frustration (@sec:sprint2test). This was due to testing revealing that the prior implementation caused frustration (@sec:sprint2test). Since LLight-DDependent RResistors (LDR) were available @arduinotechdk_lys_2025 @arduinotechdk_lys_2025 they were selected for this purpose due to their relative simplicity and familiarity.

The idea behind the detection mechanism was placing ann LED at the bottom of the holder for the NFC reader and the LDR through a small hole from the top side of the holder. When no card was present, the LED light would pass unobstructed to the LDR. However, when a card was inserted, it would block the light, causing the LDR's resistance to change. This shift in light intensity could then be detected in software, triggering the system to initiate an NFC card scan automatically.
...
#figure(
  image("../images/sprint 4/pcb4solderedtop.jpg", heghheight: 3333%),
...
=== Molex Connectors
=== Molex Connectors
Molex @molex_creating_2025 connectors were used to connectoff-board components, such as the USB port and power switch, to the PCB (@fig:molexConnectors). They were used because they providedd secure and quick-release capability, simplifying assembly, disassembly and maintenance of the product. Although the NFC reader was originally planned to use a similar connector, it proved more practical to connect a pin-header @sullins_connector_solutions_100_nodate to its pins and solder flexible wires between the header and the PCB (@fig:nfcHeader). This approach still alloweded the reader to be unplugged for removing the PCB without disturbing the rest of the wiring.
...
#subpar.grid(
  ...
  align: top,
  figure(image("../images/sprint 4/molex4power.jpg", height: 2020%),
    caption: [Molex connection for power button.]), <fig:molexConnectors>,
  figure(image("../images/sprint 4/molex4nfc.jpg", height: 2020%),
...
== Testing <sec:test4>
The fourth round of user testing consisted of observing how participants interacted with a functional assembled Controller (@fig:sprint4TestSetup). However, as previously discussed((@potsprint4)), issues with the multiplexer prevented the potentiometers and the automatic NFC reader sensor from working correctly. To make the NFC reader work, one of the interactive buttons was reconfigured to manually activate it. The potentiometers were disabled. The test was conducted at the Danish primary school, the Danish primary school, Rosengårdskolen @rosengardskolen_velkommen_nodate,, and involved four individual participants from the 5th grade. Each session lasted approximately seven minutes. A/B testing, think-aloud methodology, and unstructured interviews were employed.
...
#subpar.grid(
  ...
  align: top,
  gutter: 30pt,
  figure(image("../images/sprint 4/test4setup.jpg", height: 223%),
    caption: [Full ttest ssetup.]), <fig:sprint4TestSetup>,
  figure(image("../images/sprint 4/test4pots.jpg", height: 223%),
    caption: [Knob A/B test.]), <fig:sprint4TestKnobs>,
    placement: bottom,
    placement: bottom
)

Two participants reported having prior formal musicalal training. One played the transverse flute and another the guitar; the remaining two had no previous musical experience.
...
Two A/B tests were conducted. One using different side panel designs featuring varying slit szesizes for NFC card insertion. It became clear that one version was too narrow, as multiple participants reported difficulty inserting the cards. Furthermore, it was observed that participants did not find it intuitive to insert the NFC cards into the chassis.
