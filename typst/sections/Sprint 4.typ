...
Reading the potentiometer values worked by continuously iterating through an array containing the binary multiplexer settings for each potentiometer, setting the multiplexer to let the specific potentiometer's signal through one at a time (@listing:sprint4Pot:18).
...
=== Molex Connectors
Molex @molex_creating_2025 connectors were used to connect off-board components, such as the USB port and power switch, to the PCB (@fig:molexConnectors). They were used because they provided secure and quick-release capability, simplifying assembly, disassembly and maintenance of the product. Although the NFC reader was originally planned to use a similar connector, it proved more practical to connect a pin-header @sullins_connector_solutions_100_nodate to its pins and solder flexible wires between the header and the PCB (@fig:nfcHeader). This approach still allowed the reader to be unplugged for removing the PCB without disturbing the rest of the wiring.
...
== Testing <sec:test4>
The fourth round of user testing consisted of observing how participants interacted with a functional assembled Controller (@fig:sprint4TestSetup). However, as previously discussed (@potsprint4), issues with the multiplexer prevented the potentiometers and the automatic NFC reader sensor from working correctly. To make the NFC reader work, one of the interactive buttons was reconfigured to manually activate it. The potentiometers were disabled. The test was conducted at the Danish primary school, Rosengårdskolen @rosengardskolen_velkommen_nodate, and involved four individual participants from the 5th grade. Each session lasted approximately seven minutes. A/B testing, think-aloud methodology, and unstructured interviews were employed.
...
Two A/B tests were conducted. One using different side panel designs featuring varying slit sizes for NFC card insertion. It became clear that one version was too narrow, as multiple participants reported difficulty inserting the cards. Furthermore, it was observed that participants did not find it intuitive to insert the NFC cards into the chassis.
