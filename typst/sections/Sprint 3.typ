...
The wired solution offered two key advantages that outweighed its drawbacks. First, it removed nearly all latency, delivering an immediate response that felt more natural and expected. Second, it enabled the Controllers to draw power directly from the Host rather than relying on batteries. Lastly, removing the batteries not only heightened the Controller's portability (Requirement 2, @table:usabilityRequirements) making it lighter, but also reduced product maintenance for users.

==== Process
For managing the now wired connection, Inter-Integrated Circuit (I²C) was used. Though SPI had been faster @campbell_basics_2016 it would also have required another SPI bus on the Pico 1 which was not possible after allocating the NFC reader and the display one SPI bus each. Furthermore, this would require four pins on each Pico 1 and three pins plus one additional pin for each Controller connected to the Host @campbell_basics_2016-1 whereas I²C only required two pins on each Pico @campbell_basics_2016.
