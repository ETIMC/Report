...
The wired solution offered two key advantages that outweighed its drawbacks. First, it removed nearly all latency, delivering an immediate response that felt more natural and expected. Second, it enabled the Controllers to draw power directly from the Host rather than relying on batteries. Lastly, removing the batteries also reduced product maintenance for users.

==== Process
For managing the now wired connection, I²C was used. Though SPI had been faster @campbell_basics_2016 it would also have required four pins on each Pico 1 and three pins plus one additional pin for each Controller connected to the Host @campbell_basics_2016-1 whereas I²C only required two pins on each Pico @campbell_basics_2016.
...
During internal testing a second Pico 1 was connected to briefly test, how the Host handled having two Controllers connected. The test went well apart from one problem. The Host could not differentiate between the Controllers. After debugging it was discovered that both Controllers had the same I²C address, which was caused by the code being copied from one Controller to the other. After changing this address on the second Controller, the two Controllers were able to communicate flawlessly with the Host without any noticeable latency or delay.

==== Physical interface <sec:sprint3PhysicalInterface>
Standard USB 2.0 Type B to A cables were chosen to physically link the Controllers and Host because they were #quote[designed to receive data, power, and control signals from a host device.] @cooper_is_2024. Furthermore, they were both available and affordable during development. They were also more robust and easier to plug in than smaller connectors @cooper_decoding_2024, which was believed would heighten the portability and Plug'N'Play usability of the system (Requirement 2 & 6, @table:usabilityRequirements). Most importantly, each cable carries exactly four conductors, and each connector exposes exactly four pins. This meant that two pins could be reserved for power (+5V and ground), while the two other pins could be reserved for the two required I²C pins: Serial Data Line (SDA) and Serial Clock Line (SCL).

Beyond pin count, USB Type B connectors offered good mechanical retention and strain relief, helping to prevent accidental disconnections during use. Standard cable lengths (up to 5 meters for USB 2.0) provided good reach, mitigating some of the flexibility downsides of switching from wireless connectivity. Lastly, by relying on off-the-shelf USB cables rather than custom cables, the design minimized part variety, simplified sourcing and replacement, and leveraged existing technology familiar to users.
...
== PCB <sec:PCBsprint3>
=== Design
To develop a fully functional prototype beyond a breadboard implementation, a PCB was designed. During this process, practical considerations such as the placement of mounting holes for integration within the final chassis were taken into account, which meant that the size of the PCB itself was determined by the size of the chassis. Additionally, due to the use of specific components that lacked readily available footprints, several custom or modified footprints were created as part of the PCB design.

The GitHub repository KiCad-RP-Pico @ncarandini_kicad-rp-pico_2022 served as the foundation for the Pico footprint. However, since the prototype utilized Pico 1s with WiFi, minor modifications were made to the footprint. This adapted footprint also served as the template for designing the display's footprint, given that the display module shared the same number of pins as the Picos. Based on the previous test (@sec:sprint2test), it was decided to use the NuPhy switches going forward. Therefore, in addition to the other footprints, a separate GitHub repository providing various switch footprints @siderakb_key-switchespretty_2025 was referenced. This footprint was also modified to suit the project's specific needs.

The PCB (@fig:firstattemptpcb & @fig:secondattemptpcb) was initially routed using the _FreeRouting_ tool @noauthor_freeroutingfreerouting_2025, but the generated layout proved impractical for soldering. Since the board was to sit just beneath the chassis lid, for space purposes, a two-sided PCB was required: buttons, potentiometers and display on the top, all other components on the bottom. Unfortunately, FreeRouting did not honor these layer constraints, placing several critical traces, including some for button contacts, on the top copper foil where they would interfere with component pads.

To resolve this, the PCB was redesigned and all traces were drawn manually (@fig:thirdattemptpcb), ensuring that top-side copper was reserved exclusively for buttons, potentiometers and display connections and that all other routing remained on the bottom layer. Lastly, the LEDs on the schematic (@app:schematicSprint3) was omitted from the PCB design, as it was deemed more important getting the core functionality of the Controller working before adding flair despite previous tests highlighting how it could be beneficial (@sec:sprint1test & @sec:sprint2test).
...
One of the first additions was the creation of PCB standoffs; four cylindrical supports with central threaded screw holes, placed to align with the mounting holes of the PCB (@fig:fus3nfcStandoffs). Before printing, the placement and alignment of the standoffs were verified in Fusion by inserting a model of the PCB.

A dedicated holder for the NFC reader was designed (@fig:fus3nfcStandoffs). This component included a fixed position for the NFC reader and a dedicated slot for card insertion. A physical boundary, acting as a "stopper", was added inside the slot to prevent the NFC cards from being inserted too far into the chassis, both to protect other internal components and in response to user testing (@sec:sprint2test), where participants expressed curiosity about placing foreign objects into the opening. 

Two new side panels were created as variations of the original design. The first was an NFC card insertion side panel (@fig:fus3nfcside), which featured a large rectangular opening with smoothed edges to ensure safe use. The second was a rear side panel, where cutouts were added for both the USB 2.0 Type B connector and the power switch.

Lastly, a lid was designed to align with the base of the chassis and the component layout on the PCB (@fig:fus3lid). Corresponding cutouts were added for the buttons, display, and potentiometers, ensuring all interactive elements were accessible. Corresponding to the base of the chassis' magnet holes (@sec:sprint2Fusion), holes were also added to the lid design. To improve the surface finish, the lid was _ironed_ using a feature in Bambu Studio @bambu_lab_software_2025. However, the result did not meet expectations and was therefore reconsidered in later iterations.

A simple temporary attachment mechanism for the lid was introduced using small 3D-printed cylinders that fit into the designed magnet holes. This meant that the lid would not slide off the base of the chassis, before magnets were added.
...
To solve this, a new idea was tested: printing the screw without a head, and pairing it with a small plastic bolt to hold the PCB in place, which would be easier to remove than the previously printed screws. While the screw screwed into the chassis and correctly held the PCB in place, the bolt did not fit as intended. In the end, it was decided to simply rest the PCB on top of the standoffs, using only the headless screw for positioning (@fig:fus3printedchassis). Since the lid sat close enough to the PCB, even without bolts, there was no risk of the board shifting during use.
/*
skruer:
...
#subpar.grid(
  ...
  align: top,
  figure(image("../images/sprint 3/box-v1.jpeg", height: 25%),
    caption: [Mechanical turk test.]), <fig:test3Turk>,
  figure(image("../images/sprint 3/breadboardSprint3.jpg", height: 25%),
...
When asked whether the system might motivate them to explore a new musical instrument, two participants, having musical backgrounds playing drums and guitar, responded negatively. However, one participant noted: #quote[Hmm... Especially if you've never played music before, then I think it would be like that.] (@app:Sprint3Transcriptions) which suggests the product could hold great appeal for individuals without prior musical experience.
