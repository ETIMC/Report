...
Based on the breadboard prototype tests (@sec:sprint2test), two additional instruments, a trumpet and a guitar, were integrated into the system. In Live, this involved creating two new MIDI tracks, each loaded with a high-quality trumpet or guitar virtual instrument and assigned to its own dedicated MIDI channel. Correspondingly, the Controller’s instrument-selection UI was updated: two new images (one for the trumpet, one for the guitar) were designed and added to the display (@fig:pixelart2), ensuring users receive clear visual feedback whenever they switch to one of these instruments. Lastly, two new NFC cards were assigned to each of the instruments.


== Lowering latency
=== UDP
As discussed in @sec:sprint2WifiProblems, latency, primarily caused by TCP, was experienced when playing on the Controller. To improve this, the WiFi system was changed to use UDP instead.
This was mostly a simple process. Most rewriting consisted of removing TCP sockets (@listing:exchangingSocketsDiff:1) and exchanging them with the addresses of the Controllers (@listing:exchangingSocketsDiff:3). As UDP is not connection-oriented #cite(<kurose_computer_2020>,  supplement: [ch. 3.3]), this was necessary for the Host to be able to identify and remember the Controllers that connect to it.
...
To resolve this, the PCB was redesigned and all traces were drawn manually (@fig:thirdattemptpcb), ensuring that top-side copper was reserved exclusively for button, potentiometers and display connections and that all other routing remained on the bottom layer.
