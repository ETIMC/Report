...
+ Modular magnetic blocks that can be attached together to make a music-piece.
...
+ Cooperative modules for experimenting with rhythm- and music-making with flexibility by utilizing NFC cards for easy sound experimentation
  ...
  - Could possibly both be used by singular children but also together by connecting the modules. This could possibly foster cooperative learning while still maintaining engagement for the singular user.
...
+ LED screens with interactive 'faces' that jump out of joy when music is played. (could be used in multiple ideas).
+ Classic piano roll where different modules can be attached. 

The second idea, cooperative modules for music experimentation utilizing an NFC reader, with a target group of 9-12-year-olds, was selected as the basis for the project. This aligned with the 3rd-6th primary school grades, in which music is taught in the Danish school system @styrelsen_for_undervisning_og_kvalitet_faghaefte_2025. In practice, the concept involves providing multiple _Controllers_ that the target group can use to experiment and explore music with. A sketch was produced visualizing the idea (@fig:firstDrawing).
