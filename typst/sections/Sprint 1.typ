...
The last, and arguably most important, requirement, 10 (Interactive sensors), was required as this would be what the user would use to interact and interface with the Controller for experimenting with music.
...
The participant's preliminary expectations included functionality for recording and playback of predefined beats, similar to GarageBand @apple_garageband_2024, and the ability to alter pitch, intended humorously to produce annoying sounds. Although the tester was not part of the target demographic, such feedback was interpreted as indicative of an exploratory and motivated interaction approach. During the A/B test, the tester expressed a preference for NFC reader placement on the right side of the device. 
