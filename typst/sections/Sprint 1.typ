...
All technical requirements are listed in @table:technicalRequirements. Requirement 1 (MCU) was specified to balance processing power with cost-effectiveness, aligning with usability requirement 5 (@table:usabilityRequirements). Selecting a microcontroller over a full-fledged computer simplifies the device architecture while preserving flexibility, user-friendliness, and affordability. The chosen Microcontroller Unit (MCU) platform also needed built-in support for wireless communication, per requirement 4 (Wireless Connection).
...
This led to a basic breadboard setup where the reader was connected to a Pico and two differently colored LEDs, each assigned an instrument. The code was kept intentionally simple, but was adapted to map the Unique Identifiers (UID) of the NFC cards to instrument names (@listing:nfcCardLabels) in a dictionary. 
