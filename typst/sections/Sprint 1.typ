...
Requirement 4 (Wireless Connection) was defined as since there would both be a Host and multiple Controllers, they would all have to communicate. Among available options, WiFi was selected for its lower end-to-end latency compared to alternatives like Bluetooth #cite(<sharrow_speed_2025>), ensuring that the system could transmit MIDI data rapidly enough as required by requirement 9 (Low latency).
...
=== MCU
Two microcontroller families were evaluated for both the controllers and the MIDI host: the Raspberry Pi Pico series and the Arduino Uno series @raspberry_pi_ltd_buy_2025 @arduino_arduino_2025. Both offer WiFi–capable models and provide sufficient general-purpose input/output (GPIO) pins to accommodate the project’s sensors and actuators (buttons, potentiometers, NFC reader, and display). The Arduino platform presented advantages in terms of prior team familiarity, extensive library support, and a well-­established first-party IDE. However, its WiFi–enabled variants carry a significant price premium, often costing more than double that of comparable RP2040-based boards @jkollerupdk_raspberry_nodate @ardustoredk_arduino_nodate.
...
==== CircuitPython
While exploring the Raspberry Pi Pico ecosystem the CircuitPython programming language @adafruit_circuitpython_nodate was discovered. CircuitPython is based on Python and promises simplicity, quick prototyping and "strong hardware support" @schroeder_what_2024. Furthermore, CircuitPython's creator Adafruit has already created many packages and libraries for everything from interfacing with displays to Bluetooth connectivity, including packages for working with WiFi @noauthor_adafruit_2025 and MIDI @walters_adafruit_2025. While testing, a Pico 2 was successfully programmed to host a WiFi hotspot using the beforementioned library. Likewise, a Pico 1 was successfully configured to connect to the Pico 2's hotspot, and they could successfully exchange simple HTTP messages wirelessly. For experimentation purposes, using the beforementioned MIDI library, the host was successfully configured to act as a MIDI interface, sending MIDI notes on multiple MIDI channels to an instance of Live running on a USB-connected laptop. 

The primary drawback of CircuitPython was its confinement of the Pico’s execution to a single CPU core @mlewus_multicore_2021, raising concerns about multitasking performance as the host device would need to manage multiple controllers and a computer concurrently. Fortunately, the discovery of the _asyncio_ library addressed this limitation @halbert_cooperative_2022 by providing a cooperative multitasking framework, where functions explicitly yield control and later resume after a specified interval, making sure functions do not block each other.

=== Display <sprint1display>
To address the technical requirement (Requirement 6, @table:technicalRequirements) stating the product should incorporate a display, it was researched which would be the best fit. The selected component was the Pico Display Pack 2.0 @pimoroni_pico_2025, which is based on the ST7789 display driver and uses the SPI protocol for communication. This particular display was chosen for its natural compatibility with the Picos and their 3.3V power output, its compact physical dimensions, and cost-effectiveness, thereby supporting the project's goal of maintaining affordability (Requirement 5, @table:usabilityRequirements).
...
Both the _Parallax RFID Reader_ @parallaxcom_rfid_2024 and the _RFID RC522 Module_ @arduinotechdk_rfid_2025 were considered for implementing NFC functionality, both compatible with the Picos 3.3V power output. After brief testing and evaluation, the RC522 was chosen. The decision primarily rested on two factors: physical size and cost (Requirements 3 & 5, @table:usabilityRequirements). The Parallax reader measured nearly twice the size of the RC522, which made it less suitable for a compact Controller setup. In terms of cost, the Parallax reader was significantly more expensive, priced five times more than the RC522 module @arduinotechdk_rfid_2025 @parallaxcom_rfid_2024.
...
This led to a basic breadboard setup where the reader was connected to a Pico and two differently colored LEDs, each assigned an instrument. The code was kept intentionally simple, but was adapted to map the UID's of the NFC cards to instrument names (@listing:nfcCardLabels) in a dictionary. 
...
Multiple button types where explored. This included both classic switch buttons @adafruit_tactile_nodate but also force sensitive resistors @adafruit_square_2022 and piezoresistors @kosaka_e-drum_nodate, which would allow the user the play notes with different velocities. Ultimately, however, it was decided to use switch buttons, to keep the design and development process simple.
...
The participant's preliminary expectations included functionality for recording and playback of predefined beats, similar to GarageBand @apple_garageband_2024, and the ability to alter pitch, intended humorously to produce annoying sounds. Although the participant was not part of the target demographic, such feedback was interpreted as indicative of an exploratory and motivated interaction approach. During the A/B test, the participant expressed a preference for inserting the NFC cards on the right side of the device. 
